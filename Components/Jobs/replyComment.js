...
class ReplyComment extends Component {
    ...
    onDelete() {
        const { onDeleteJobReplyLoading, deleteJobReplyApi, reply, job, comment, token } = this.props;
        onDeleteJobReplyLoading();
        const body = {
            'jobId': job._id,
            'commentId': comment._id,
            'replyId': reply._id
        }
        deleteJobReplyApi(body, token);
    }

    render() {
        const { reply, deleteJobReplyLoading, currentUser } = this.props;
            ...
            <View style={styles.replyContainer}>
                <View style={styles.commentAuthor}>
                    ...
                    {isUserReply && <TouchableOpacity onPress={() => this.confirmDeleteReply()} style={styles.flex_row}>
                        {deleteJobReplyLoading ? <ActivityIndicator style={{ marginRight: 30, paddingTop: 10 }} color={'#283E4A'} />
                            : <Icon
                                name={'trash'}
                                size={ratio * 10}
                                color={'#555'}
